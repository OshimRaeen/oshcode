...
Instructions:
1. Provide ONLY the raw code that should be inserted exactly at the |CURSOR| location.
2. Maintain proper indentation and style.
3. Follow ${context.language} best practices.
4. DO NOT wrap the response in markdown blocks (\`\`\`).
5. DO NOT provide any conversational text or explanations.`;
}

// 3. Updated function to use Groq API instead of local Ollama
  ...
  try {
    const chatCompletion = await groq.chat.completions.create({
      messages: [
        {
          role: "system",
          content: "You are an expert, precise code completion assistant. You output only raw, syntax-correct code without formatting."
        },
        {
          role: "user",
          content: prompt
      ...
      // Llama 3 8B is blazing fast and perfect for inline autocomplete
      // model: "llama3-8b-8192",depricated 
      // model: "llama-3.1-8b-instant", 
      model: "llama-3.1-8b-instant", 
      temperature: 0.1, // Lower temperature means more predictable code output
      max_tokens: 300,
    });

    let suggestion = chatCompletion.choices[0]?.message?.content || "";

    // Clean up the suggestion just in case the AI ignores instructions and adds markdown
